...
public class LoginX extends JavaPlugin implements Listener {
    ...
    public void onEnable() {
        ...
        // Kasa özel renklerini yükle
        if (getConfig().contains("crateColors")) {
            for (String name : getConfig().getConfigurationSection("crateColors").getKeys(false)) {
    ...
    public void onKontQuit(PlayerQuitEvent ev) {
        ...
        kontUntil.remove(uid);
        kontYetkilisi.remove(uid);
        kontSavedInv.remove(uid); // eşyalar kaybedilmesin — banlı olduğu için zaten geri giremez
