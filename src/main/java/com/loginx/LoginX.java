...
public class LoginX extends JavaPlugin implements Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        // /izinver — konsol veya op — menü aç
        if (cmdName.equals("izinver")) {
            ...
            if (args.length < 1) {
                if (sender instanceof Player p) openIzinMenu(p);
                else sender.sendMessage("Kullanim: /izinver <oyuncu>");
                return true;
            }
            Player target = Bukkit.getPlayer(args[0]);
            if (target == null) { sender.sendMessage(color("&cOyuncu bulunamadı.")); return true; }
            izinliOyuncular.add(target.getUniqueId());
            getConfig().set("izinli." + target.getUniqueId().toString(), true); saveConfig();
            sender.sendMessage(color("&#00ff88&l[İZİN] &f" + target.getName() + " &7TNT/komut izni verildi."));
    ...
    public void onTNTPlace(org.bukkit.event.block.BlockPlaceEvent evB) {
        ...
        if (mat == Material.TNT || mat == Material.MINECART || mat == Material.TNT_MINECART) {
            ...
            p.sendMessage(color("&#ff4444&l[İZİN] &fTNT/Minecart koyma izniniz yok! &e/izinver &fyazın."));
        }
    }
