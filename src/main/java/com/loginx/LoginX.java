...
public class LoginX extends JavaPlugin implements Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmdName.equals("bakimaa")) {
            ...
            for (Player on : new ArrayList<>(Bukkit.getOnlinePlayers())) {
                ...
                on.kickPlayer(color("&b&lSilvera &r&fBAKIM\n&fSunucu Kısa Süreliğine Bakımdadır\n&7Lütfen Daha Sonra Tekrar Girin"));
                atilan++;
            }
    ...
    public void onPlayerMoveAyarla(PlayerMoveEvent ev) {
        ...
        for (Map.Entry<String, BoundingBox> entry : ayarlaBounds.entrySet()) {
            ...
            if (queue[0] == null) {
                ...
                playerAyarlaArena.put(p.getUniqueId(), arenaId);
                // 2 kişi var — sadece boş yerlere cam koy
                closeAyarlaArena(arenaId, bbox, p.getWorld());
                for (UUID uid : queue) {
                    Player ap = Bukkit.getPlayer(uid);
                    if (ap != null) {
                        ap.sendMessage(color(""));
                        ap.sendMessage(color("  &#00aaff&l⚔ &r&#ffffff1v1 BAŞLADI! &r&#aaaaaa&oİyi şanslar."));
                        ap.sendMessage(color(""));
    ...
    private void closeAyarlaArena(String arenaId, BoundingBox bbox, World world) {
        // Sadece boş (AIR) olan kenarlık bloklarına cam koy — var olan blokları değiştirme
        ...
        for (int x = minX; x <= maxX; x++)
            for (int y = minY; y <= maxY; y++)
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    if (edge && world.getBlockAt(x,y,z).getType() == Material.AIR)
    ...
    public void onJoin(PlayerJoinEvent e) {
        ...
        if (bakimModu && !p.isOp() && !p.hasPermission("loginx.admin") && !bakimIzinli.contains(p.getUniqueId())) {
            ...
            new BukkitRunnable() {
                @Override public void run() {
                    p.kickPlayer(color("&b&lSilvera BAKIM\n&fSunucu Kısa Süreliğine Bakımdadır\n&7Lütfen Daha Sonra Tekrar Girin"));
                }
            }.runTaskLater(this, 1L);
            return;
        }

        // ─── Bakımdayken girdi — 3 satır title göster ───
        if (bakimModu && (p.isOp() || p.hasPermission("loginx.admin") || bakimIzinli.contains(p.getUniqueId()))) {
            new BukkitRunnable() {
                @Override public void run() {
                    ...
                    p.sendTitle(
                        ...
                        color("&#ffffff Sunucu Bakımdadır"),
                        5, 80, 20);
                    p.sendMessage(color(""));
                    p.sendMessage(color("  &#ffcc00&l⚙ &r&#ffffff&lBakım Modu &r&#ffcc00&l⚙"));
                    p.sendMessage(color("  &#aaaaaa Sunucu şu an bakım modundadır."));
                    p.sendMessage(color("  &#ff4444» &e/bakimdancikar &fkomutu ile çıkabilirsin."));
                    p.sendMessage(color(""));
                }
            }.runTaskLater(this, 20L);
        }
    ...
    private void startDuel(Player p1, Player p2, String arenaId, boolean webOn, boolean elyOn) {
        ...
        // GameMode'u ASLA değiştirme
        // Sadece yürüme + uçuş hızını sıfırla (freeze — zıplama PlayerMoveEvent ile engelleniyor)
        for (Player dp : new Player[]{p1, p2}) {
            dp.setWalkSpeed(0f);
            dp.setFlySpeed(0f);
            // Title yok — sadece chat
            dp.sendMessage(color(""));
            dp.sendMessage(color("  &#00aaff&l⚔ DUEL BAŞLIYOR!"));
            dp.sendMessage(color("  &#ffffff&o3 saniye içinde serbest kalacaksın."));
            dp.sendMessage(color(""));
            ...
            @Override public void run() {
                if (count > 0) {
                    for (Player dp : new Player[]{p1, p2}) {
                        ...
                        dp.sendMessage(color("  &#ffcc00&l" + count + "..."));
                    ...
                    for (Player dp : new Player[]{p1, p2}) {
                        ...
                        dp.sendMessage(color("  &#00ff88&l⚔ SERBEST! İyi şanslar!"));
    ...
    private void endDuel(String arenaId, Player winner, Player loser) {
        ...
        if (winner != null && winner.isOnline()) {
            ...
            winner.sendMessage(color("  &#ffcc00&l⚔ KAZANDIN! &r&#ffffff" + (loser != null ? loser.getName() : "?") + " &7elendi."));
            winner.sendMessage(color(""));
            winner.playSound(winner.getLocation(), Sound.UI_TOAST_CHALLENGE_COMPLETE, 1f, 1.2f);
            // Anında spawn'a ışınla (0 sn)
            Location spawnLoc = winner.getWorld().getSpawnLocation();
            if (Bukkit.getServer().getPluginManager().getPlugin("Essentials") != null) {
                Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "essentials:spawn " + winner.getName());
            } else {
        ...
        if (loser != null && loser.isOnline()) {
            ...
            loser.sendMessage(color(""));
            loser.sendMessage(color("  &#ff4444&l⚔ KAYBETTİN. &r&#aaaaaa&oKazanan: " + (winner != null ? winner.getName() : "?")));
            loser.sendMessage(color(""));
            Location spawnLoc2 = loser.getWorld().getSpawnLocation();
            if (Bukkit.getServer().getPluginManager().getPlugin("Essentials") != null) {
                Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "essentials:spawn " + loser.getName());
            } else {
