...
public class LoginX extends JavaPlugin implements Listener {
    ...
    private void openLootChest(Player p, ItemStack chestItem, int slot) {
        ...
        // 45 slot (5 satır): 0-35 ana envanter, 36-39 zırh, 40 offhand — geri kalan slotlar boş
        Inventory gui = Bukkit.createInventory(new LootChestHolder(slot), 45, duelFlop("Düel Sandığı", false));
        for (int i = 0; i < 41 && i < contents.length; i++) gui.setItem(i, contents[i]);
        p.openInventory(gui);
    ...
    public void onLootChestClose(InventoryCloseEvent ev) {
        ...
        // İçindeki tüm eşyalar alınmışsa (sandık boşsa) sandığı tamamen kaldır
        boolean empty = true;
        for (ItemStack it : updated) {
    ...
    // ─── "kutu" dünyasında blok kırılınca item düşmesin (sadece o dünyada) ───
    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
    public void onKutuWorldBlockBreak(BlockBreakEvent ev) {
        if (ev.getBlock().getWorld().getName().equalsIgnoreCase("kutu")) {
