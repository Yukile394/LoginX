...
public class LoginX extends JavaPlugin implements Listener {
    ...
    private final HashMap<UUID, Boolean>     duelStickEnabled  = new HashMap<>(); // çubuk (ince odun) kullan mı
    ...
    // Aktif dueldeki kurallar: arenaId -> [web, ely, merdiven, meşe, çubuk, göğüslük]
    ...
    private void sendDuelInvite(Player p, Player target, String rawName) {
        ...
        target.sendMessage(color("  &7Meşe Odun:"+ (oakOn    ? " &#00ff88Açık" : " &#ff4444Kapalı")
                          + "   &7Çubuk:"       + (stickOn  ? " &#00ff88Açık" : " &#ff4444Kapalı")
    ...
    private void fillDuelToggles(Inventory inv, UUID uid, HashMap<UUID, Boolean> webMap, HashMap<UUID, Boolean> elyMap,
        ...
        inv.setItem(24, buildDuelToggleItem("Çubuk",        "stick",  Material.STICK,           Material.BARRIER,         stickOn,  "Düelloda çubuk kullanımı"));
    ...
    // ── Duel kuralları: çubuk (ince odun) ve göğüslük kapalıysa kullanılamasın ──
    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
    public void onDuelStickUse(PlayerInteractEvent ev) {
        Player dPl = ev.getPlayer();
        String dArena = playerDuel.get(dPl.getUniqueId());
        if (dArena == null) return;
        boolean[] rules = duelArenaRules.get(dArena);
        if (rules == null) return;
        ItemStack hand = ev.getItem();
        if (hand == null) return;
        if (hand.getType() == Material.STICK && !rules[4]) {
            ev.setCancelled(true);
            dPl.sendActionBar(color("&#ff4444✖ &fBu duelde çubuk kapalı!"));
        }
    }
