...
public class LoginX extends JavaPlugin implements Listener {
    ...
    // Her anahtar için özel renk paleti (isim → renkler dizisi)
    private static final Map<String, String[]> KEY_COLORS = new LinkedHashMap<>();
    static {
        // Afk Key  → Gri & Beyaz
        KEY_COLORS.put("afk",     new String[]{"&#aaaaaa", "&#cccccc", "&#ffffff", "&#cccccc", "&#aaaaaa", "&#888888"});
        // Casino Key → Kırmızı & Altın
        KEY_COLORS.put("casino",  new String[]{"&#ff2200", "&#ff6600", "&#ffaa00", "&#ffdd00", "&#ff6600", "&#ff2200"});
        // Gear Key   → Turuncu & Sarı
        KEY_COLORS.put("gear",    new String[]{"&#ff8800", "&#ffaa00", "&#ffdd00", "&#ffaa00", "&#ff8800", "&#ff5500"});
        // Booster Key → Yeşil & Açık Yeşil
        KEY_COLORS.put("booster", new String[]{"&#00aa44", "&#00cc66", "&#00ff88", "&#00cc66", "&#00aa44", "&#008833"});
        // Key All    → Mor & Cyan (premium)
        KEY_COLORS.put("all",     new String[]{"&#aa00ff", "&#cc44ff", "&#00ccff", "&#cc44ff", "&#aa00ff", "&#7700cc"});
    }
    ...
    public void onCrateInteract(PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.LEFT_CLICK_BLOCK) {
            ...
            if (!inHand.hasItemMeta() || !inHand.getItemMeta().getPersistentDataContainer().has(keyTag, PersistentDataType.STRING)) {
                ...
                p.getWorld().spawnParticle(Particle.LARGE_SMOKE, loc.clone().add(0.5, 1, 0.5), 10, 0.2, 0.2, 0.2, 0.05);
            ...
            p.getWorld().spawnParticle(Particle.TOTEM_OF_UNDYING, loc.clone().add(0.5, 1.5, 0.5), 50, 0.5, 0.5, 0.5, 0.1);
    ...
    private void openKeyMenu(Player p) {
        ...
        // Her anahtar kendi renklerinde animasyonlu olacak (updateAnimatedMenus'da işlenir)
        inv.setItem(11, createKey(Material.GRAY_DYE,       "Afk Key",     "afk"));
        inv.setItem(12, createKey(Material.RED_DYE,        "Casino Key",  "casino"));
        inv.setItem(13, createKey(Material.ORANGE_DYE,     "Gear Key",    "gear"));
        inv.setItem(14, createKey(Material.LIME_DYE,       "Booster Key", "booster"));
    ...
    private ItemStack createKey(Material m, String name, String id) {
        ...
        // İsmi renkli animasyonlu yaz (başlangıç tick=0)
        meta.setDisplayName(getKeyAnimText(name, id, 0));
        meta.getPersistentDataContainer().set(keyTag, PersistentDataType.STRING, id);
        // Lore'a key id'yi gizli sakla (animasyon güncellemesi için)
    ...
    private void openIadeMenu(Player p) {
        Inventory inv = Bukkit.createInventory(null, 54, color("&7Fear Craft - &fİade"));
    ...
    /**
     * Her anahtar için kendi renk paletinde karakter-karakter kayan animasyon.
     * Gökkuşağı değil — her anahtar sadece kendi renk dizisini kullanır.
     */
    private String getKeyAnimText(String text, String keyId, int tick) {
        String[] colors = KEY_COLORS.getOrDefault(keyId.toLowerCase(),
                new String[]{"&#ffffff", "&#cccccc", "&#aaaaaa", "&#cccccc", "&#ffffff", "&#dddddd"});
    ...
    private void updateAnimatedMenus() {
        for (Player p : Bukkit.getOnlinePlayers()) {
            ...
            if (!title.contains("Anahtar Menüsü")) continue;
            ...
            for (int slot = 11; slot <= 15; slot++) {
                ...
                // Lore'daki gizli key id'yi oku
                if (lore == null || lore.isEmpty()) continue;
                String keyId = ChatColor.stripColor(lore.get(0));

                // Temiz (renksiz) ismi KEY_COLORS map'inden değil, gömülü durumdan al
                // Mevcut display name'in renksiz halini esas isim olarak kullan
                String baseName = ChatColor.stripColor(meta.getDisplayName());
    ...
    private void updateCrateEffects() {
        for (Map.Entry<String, Location> entry : crates.entrySet()) {
            if (crateEffects.getOrDefault(entry.getKey(), false)) {
                ...
                loc.getWorld().spawnParticle(Particle.FIREWORK, loc, 1, 0, 0, 0, 0);
