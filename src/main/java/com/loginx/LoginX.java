...
public class LoginX extends JavaPlugin implements Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmdName.equals("bakima") || cmdName.equals("bakimaal")) {
            ...
            if (!p.hasPermission("loginx.admin")) { p.sendMessage(color("&cYetkisiz.")); return true; }
            if (bakimModu) {
                p.sendMessage(buildSilveraTag() + color(" &fSunucu zaten bakım modunda."));
                return true;
            }
            bakimModu = true;
        ...
        if (cmdName.equals("bakimdancikar")) {
            ...
            if (!p.hasPermission("loginx.admin")) { p.sendMessage(color("&cYetkisiz.")); return true; }
            if (!bakimModu) {
                p.sendMessage(buildSilveraTag() + color(" &fSunucu zaten bakımda değil."));
                return true;
            }
            bakimModu = false;
            for (Player on : Bukkit.getOnlinePlayers())
        ...
        if (cmdName.equals("bakimaoyuncu") || cmdName.equals("bakimaoyuncual")) {
            if (!(sender instanceof Player p)) return true;
            if (!p.hasPermission("loginx.admin")) { p.sendMessage(color("&cYetkisiz.")); return true; }
        ...
        // /cooldownhiziayarla — elindeki item vurma hızını ayarla
    ...
    public void onCitizensNPCClick(NPCRightClickEvent ev) {
        ...
        for (String cmdStr : npc.commands) {
            ...
            // Console üzerinden çalıştır — Citizens plugin event çakışmasını önler
            final String toRun = finalCmd;
