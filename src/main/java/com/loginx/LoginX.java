...
public class LoginX extends JavaPlugin implements Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        // /stashac — stash'ı aç (ölünce düşer)
        if (cmdName.equals("stashac")) {
            if (!(sender instanceof Player p)) return true;
        ...
        if (cmdName.equals("cooldownhiziayarla")) {
            ...
            if (speed <= 0 || speed > 10) { p.sendMessage(color("&cHız 0.1-10 arasında olmalı.")); return true; }
            ...
            // Görsel feedback
            String speedBar = "&#" + (speed <= 3 ? "00ff88" : speed <= 7 ? "ffcc00" : "ff4444");
            hm.setDisplayName(hm.hasDisplayName() ? hm.getDisplayName() : color("&f" + hand.getType().name()));
            hand.setItemMeta(hm);
            p.getInventory().setItemInMainHand(hand);
            p.sendMessage(color("&#00ff88&lCooldown &8» &fElinizde tuttuğunuz &e" + hand.getType().name() + " &fitemine " + speedBar + speed + "x &fhız ayarlandı."));
    ...
    public void onPlayerDeath(PlayerDeathEvent e) {
        ...
        // ─── Stash varsa düşür (stashAc açıksa) ───
        if (stashOpen.getOrDefault(dead.getUniqueId(), false)) {
            ...
            if (stashContents != null) {
                ...
                dead.getWorld().dropItem(dead.getLocation(), stashShulker).setPickupDelay(40);
    ...
    // ─── Yere Düşen Item İsim Hologramı ───
    // İtem spawn olunca üstüne isim etiketli ArmorStand koy (blok arkasından gözükmez)
    @EventHandler
    public void onItemSpawn(ItemSpawnEvent ev) {
        org.bukkit.entity.Item dropped = ev.getEntity();
        ItemStack is = dropped.getItemStack();
        if (is == null || is.getType() == Material.AIR) return;
        // İsim ve rengi al
        String displayName;
        if (is.hasItemMeta() && is.getItemMeta().hasDisplayName()) {
            displayName = is.getItemMeta().getDisplayName(); // renkli isim aynen kullan
        } else {
            // Standart item — formatla
            displayName = color("&#aaaaaa" + formatMaterial(is.getType()));
        }
        // 1 tick sonra ArmorStand yerleştir (item fizik yerine otururken)
        final String fname = displayName;
        new BukkitRunnable() {
            @Override public void run() {
                if (!dropped.isValid()) return;
                Location loc = dropped.getLocation().clone().add(0, 0.3, 0);
                ArmorStand as = (ArmorStand) dropped.getWorld().spawnEntity(loc, EntityType.ARMOR_STAND);
                as.setVisible(false);
                as.setGravity(false);
                as.setSmall(true);
                as.setArms(false);
                ...
                as.setCustomName(fname);
                as.setCustomNameVisible(true);
                // Item silinince ArmorStand'ı da sil
                new BukkitRunnable() {
                    @Override public void run() {
                        if (!dropped.isValid()) {
                            as.remove();
                            ...
                            as.teleport(dropped.getLocation().clone().add(0, 0.3, 0));
                        }
                    }
                }.runTaskTimer(LoginX.this, 1L, 2L);
            }
        }.runTaskLater(this, 1L);
    }
