...
public class LoginX extends JavaPlugin implements Listener {
    ...
    // ─── Büyük görünen (ItemDisplay) shulker düşme takibi: display uuid -> itemstack ───
    private final HashMap<UUID, ItemStack> bigDropContents = new HashMap<>();
    ...
    public void onEnable() {
        ...
        // ─── Büyük (ItemDisplay) shulker pickup kontrolü — her 5 tickte bir yakındaki oyuncuları kontrol et ───
        new BukkitRunnable() {
            @Override public void run() {
                if (bigDropContents.isEmpty()) return;
                Iterator<Map.Entry<UUID, ItemStack>> it2 = bigDropContents.entrySet().iterator();
                while (it2.hasNext()) {
                    Map.Entry<UUID, ItemStack> entry = it2.next();
                    org.bukkit.entity.Entity ent = Bukkit.getEntity(entry.getKey());
                    if (ent == null || ent.isDead() || !ent.isValid()) { it2.remove(); continue; }
                    Location eloc = ent.getLocation();
                    for (Player pl : eloc.getWorld().getPlayers()) {
                        if (pl.getLocation().distanceSquared(eloc) <= 2.25) { // ~1.5 blok yarıçap
                            HashMap<Integer, ItemStack> leftoverBig = new HashMap<>(pl.getInventory().addItem(entry.getValue().clone()));
                            if (leftoverBig.isEmpty()) {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmdName.equals("1v1")) {
            ...
            if (playerDuel.containsKey(p.getUniqueId())) { p.sendMessage(color("&#ff4444&l[1v1] &cZaten bir dueldasın.")); return true; }
            if (playerDuel.containsKey(target.getUniqueId())) { p.sendMessage(color("&#ff4444&l[1v1] &c" + target.getName() + " zaten bir duelda.")); return true; }
            ...
            if (duelInvite.containsKey(target.getUniqueId())) {
                ...
                if (existing.equals(p.getUniqueId())) {
                    p.sendMessage(color("&#ff4444&l[1v1] &cZaten bu oyuncuya davet gönderdin. Kabul etmesini bekle."));
                    return true;
                }
            }

            String freeArena = findFreeDuelArena();
            if (freeArena == null) { p.sendMessage(color("&#ff4444&l[1v1] &cŞu an boş arena yok!")); return true; }
            ...
            int tid = new BukkitRunnable() {
                @Override public void run() {
                    ...
                    if (p.isOnline()) p.sendMessage(color("&#ff4444&l[1v1] &f" + target.getName() + " &7daveti reddetti/süresi doldu."));
                    if (target.isOnline()) target.sendMessage(color("&#ff4444&l[1v1] &f" + p.getName() + " &7ile duel daveti süresi doldu."));
                }
            ...
            target.sendMessage(color("  &#00aaff&l⚔ &r&#ffffff&l" + p.getName() + " &r&#aaaaaa&oseni 1v1'e davet etti!"));
            ...
            p.sendMessage(color("&#00aaff&l[1v1] &f" + target.getName() + " &7kişisine duel daveti gönderildi."));
            return true;
        }
    ...
    /**
     * Oyuncunun envanterini mavi shulker kutusuna koyar.
     * Eşyalar sığmazsa ikinci (ve gerekirse daha fazla) shulker açılır.
     * Sadece duel / 1v1 ayarla arena ölümlerinde çağrılır.
     */
    private void dropDuelShulker(Player p, Location loc) {
        List<ItemStack> normalItems = new ArrayList<>();
        List<ItemStack> existingShulkers = new ArrayList<>(); // envanterde zaten olan shulker'lar — kendi başlarına düşecek
        for (ItemStack it : p.getInventory().getContents()) {
            if (it == null || it.getType() == Material.AIR) continue;
            // ── Nested shulker fix: oyuncunun envanterindeki shulker box'ları
            // yeni "Duel Eşyaları" kutusunun İÇİNE koymuyoruz, ayrı bir item olarak düşürüyoruz ──
            if (it.getType().name().contains("SHULKER_BOX")) {
                existingShulkers.add(it.clone());
            } else {
                normalItems.add(it.clone());
            }
        }
        p.getInventory().clear();

        // Envanterdeki mevcut shulker'ları olduğu gibi, kendi ayrı item'ları olarak büyük düşür
        for (ItemStack existing : existingShulkers) {
            dropBigShulker(loc, existing);
        }

        if (normalItems.isEmpty()) return;

        int idx = 0;
        while (idx < normalItems.size()) {
            ItemStack shulkerItem = new ItemStack(Material.BLUE_SHULKER_BOX);
            BlockStateMeta bsm = (BlockStateMeta) shulkerItem.getItemMeta();
            ShulkerBox box = (ShulkerBox) bsm.getBlockState();
            bsm.setDisplayName(color("&#0077ff&lDuel Eşyaları"));
            bsm.setLore(new ArrayList<>());

            Inventory boxInv = box.getInventory();
            int filled = 0;
            while (idx < normalItems.size() && filled < boxInv.getSize()) {
                boxInv.setItem(filled, normalItems.get(idx));
                idx++; filled++;
            }
            bsm.setBlockState(box);
            shulkerItem.setItemMeta(bsm);

            dropBigShulker(loc, shulkerItem);
        }
    }

    /**
     * Shulker'ı normalden büyük görünecek şekilde düşürür (ItemDisplay ile).
     * Gerçek Item entity yerine büyütülmüş görsel bir ItemDisplay kullanılır;
     * yakına gelen oyuncu otomatik alır (bigDropContents üzerinden periyodik kontrol).
     */
    private void dropBigShulker(Location loc, ItemStack shulkerItem) {
        org.bukkit.entity.ItemDisplay display = (org.bukkit.entity.ItemDisplay) loc.getWorld().spawnEntity(
            loc.clone().add(0, 0.2, 0), org.bukkit.entity.EntityType.ITEM_DISPLAY);
        display.setItemStack(shulkerItem);
        display.setGravity(true);
        org.bukkit.util.Transformation tr = new org.bukkit.util.Transformation(
            new org.joml.Vector3f(0f, 0f, 0f),
            new org.joml.Quaternionf(),
            new org.joml.Vector3f(1.7f, 1.7f, 1.7f),
            new org.joml.Quaternionf());
        display.setTransformation(tr);
        bigDropContents.put(display.getUniqueId(), shulkerItem);
    }
    ...
    private void openDuelSettingsMenu(Player p, boolean isAdmin) {
        Inventory inv = Bukkit.createInventory(null, 27, color("&#0055cc&l⚔ Duel Ayarları"));
        ItemStack fill = makeBorderGlass("&#0a0a1e");
        ...
        hm.setDisplayName(color("&#00aaff&l⚔ &r&#ffffff&lDuel Ayarları"));
    ...
    public void onDuelQuit(PlayerQuitEvent evq) {
        ...
        // Eşyaları shulker'a koy (duel için) ve yere düşür
        dropDuelShulker(p, p.getLocation());
    ...
    public void onDuelCommand(PlayerCommandPreprocessEvent evCmd) {
        ...
        // /ec, /enderchest, /pv, /pv 1, /pv 2, /pvault, /1v1reddet, /duelberabere ve türevleri
        boolean allowed = cmd.equals("ec") || cmd.equals("enderchest")
            || cmd.equals("pvault") || cmd.equals("1v1reddet")
            || cmd.equals("pv") || fullCmd.startsWith("pv ")
            || cmd.equals("duelberabere") || cmd.equals("duelberaberekabul") || cmd.equals("duelberaberereddet");
        if (!allowed) {
            evCmd.setCancelled(true);
            p.sendMessage(color("  &#ff4444&l[DUEL] &fDuel sırasında sadece &e/ec &f, &e/pv 1 &f, &e/pv 2 &f, &e/duelberabere &fkomutlarını kullanabilirsin!"));
        }
    }
    ...
    public void onChat(org.bukkit.event.player.AsyncPlayerChatEvent ev) {
        ...
        if (hasKufur) {
            ...
            if (warnK >= 5) {
                ...
                p.sendMessage(silvera() + color(" ") + boldFlop("KÜFÜR Uyarısı") + color(" &85/5 &8— &c10 dakika susturuldun!"));
                for (Player on : Bukkit.getOnlinePlayers())
                    if (on.hasPermission("loginx.admin"))
                        on.sendMessage(silvera() + color(" &f" + p.getName() + " &7→ küfür 10 dk mute."));
            } else {
                p.sendMessage(silvera() + color(" ") + boldFlop("KÜFÜR Uyarısı") + color(" &8" + warnK + "/5"));
            }
            return;
        }
        ...
        if (times.size() >= SPAM_THRESHOLD) {
            ...
            if (warnS >= 5) {
                ...
                times.clear();
                p.sendMessage(silvera() + color(" ") + boldFlop("SPAM Uyarısı") + color(" &85/5 &8— &c10 dakika susturuldun!"));
                for (Player on : Bukkit.getOnlinePlayers())
                    if (on.hasPermission("loginx.admin"))
                        on.sendMessage(silvera() + color(" &f" + p.getName() + " &7→ spam 10 dk mute."));
            } else {
                p.sendMessage(silvera() + color(" ") + boldFlop("SPAM Uyarısı") + color(" &8" + warnS + "/5"));
            }
        }
    }
