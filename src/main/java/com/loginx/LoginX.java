...
public class LoginX extends JavaPlugin implements Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmdName.equals("1v1")) {
            ...
            // ── RGB mavi-beyaz flop düğmeler — sadece [1v1 Kabul] [1v1 Reddet] ──
            net.md_5.bungee.api.chat.TextComponent btnKabul = new net.md_5.bungee.api.chat.TextComponent(
                buildRgbFlop("[1v1 Kabul]", new String[]{"&#00aaff","&#44ccff","&#88eeff","&#ffffff","&#88eeff","&#44ccff","&#00aaff"}, true));
            ...
            net.md_5.bungee.api.chat.TextComponent btnReddet = new net.md_5.bungee.api.chat.TextComponent(
                buildRgbFlop("[1v1 Reddet]", new String[]{"&#ff4444","&#ff7744","&#ffaa44","&#ffffff","&#ffaa44","&#ff7744","&#ff4444"}, true));
            ...
            if (queue[0] == null) {
                ...
                for (UUID uid : queue) {
                    ...
                    if (ap != null) {
                        ...
                        ap.sendMessage(color("  &#aaaaaa&o⚠ Oyundan çıkarsan ölürsün!"));
                        ap.sendMessage(color(""));
    ...
    public void onPlayerDeath(PlayerDeathEvent e) {
        ...
        // ─── Envanteri tek noktaya dök (dağılmadan) ───
        ...
        if (playerAyarlaArena.containsKey(dead.getUniqueId())) {
                        ...
                        if (sec > 0) {
                            ...
                            if (finalWinner != null && finalWinner.isOnline()) {
                                finalWinner.sendTitle(
                                    ...
                                    0, 25, 5);
                                finalWinner.sendMessage(color("  " + duelFlop("Camlar " + sec + " saniyede açılıyor...", false)));
            ...
            ayarlaOriginalBlocks.remove(arenaId);
        }
    }
    ...
    // Duel RGB flop mavi/açık mavi renkleri
    private static final String[] DUEL_BLUE_COLS = {"&#0077ff","&#0099ff","&#00bbff","&#00ddff","&#44eeff","&#88f4ff","&#ffffff","&#88f4ff","&#44eeff","&#00ddff","&#00bbff","&#0099ff"};

    private String duelFlop(String text, boolean bold) {
        return buildRgbFlop(text, DUEL_BLUE_COLS, bold);
    }
    ...
    private void teleportToSpawn(Player p) {
        if (Bukkit.getServer().getPluginManager().getPlugin("Essentials") != null) {
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "essentials:spawn " + p.getName());
        } else {
    ...
    public void onDuelQuit(PlayerQuitEvent evq) {
        ...
        // Bağlantı kesen direk ölür, diğeri kazanır — dupe yok
        UUID[] dp = duelPlayers.get(arenaId);
        if (dp == null) return;
        Player winner = null;
        for (UUID u : dp) if (!u.equals(p.getUniqueId())) winner = Bukkit.getPlayer(u);

        // Çıkan oyuncuyu direkt öldür (inventory düşer, dupe yok)
        if (p.isOnline()) {
            p.setHealth(0); // Ölüm eventi tetiklenir, endDuel oradan çağrılır
        } else {
            // Offline — manuel temizle
            ...
            if (winner != null && winner.isOnline()) {
                ...
                winner.playSound(winner.getLocation(), Sound.UI_TOAST_CHALLENGE_COMPLETE, 1f, 1.2f);
                // 8sn sonra spawna
                final Player fw = winner;
                new BukkitRunnable() {
                    int sec = 8;
                    @Override public void run() {
                        if (!fw.isOnline()) { cancel(); return; }
                        if (sec > 0) {
                            fw.sendTitle(duelFlop("🏆 KAZANDIN! 🏆", true),
                                color("&#00ccff⌛ &fSpawn'a ışınlanıyorsun: &e" + sec + " &fsaniye"), 0, 25, 5);
                            fw.sendMessage(color("  " + duelFlop("SPAWNA IŞINLANIYORSUN", false) + " &8— &e" + sec + " &fsaniye"));
                            fw.playSound(fw.getLocation(), Sound.BLOCK_NOTE_BLOCK_HAT, 0.5f, 1.5f);
                            sec--;
                        } else {
                            cancel();
                            fw.sendTitle(duelFlop("⚡ SPAWNA IŞINLANDI! ⚡", true), color("&#aaaaaa&oİyi oyunlar!"), 5, 50, 15);
                            teleportToSpawn(fw);
                        }
                    }
                }.runTaskTimer(this, 0L, 20L);
            ...
            for (Player on : Bukkit.getOnlinePlayers())
                if (on.hasPermission("loginx.admin") || on.getWorld().equals(winner != null ? winner.getWorld() : Bukkit.getWorlds().get(0)))
                    on.sendMessage(color("  " + duelFlop("⚔ DUEL SONUCU ⚔", true) + " &8— &#ff4444" + p.getName() + " &7çıktı, &#ffcc00" + (winner != null ? winner.getName() : "?") + " &7kazandı!"));
        }
    }
    ...
    public void onDuelCommand(PlayerCommandPreprocessEvent evCmd) {
        ...
        // /ec, /enderchest, /pv, /pv 1, /pv 2, /pvault, /1v1reddet
        ...
        if (!allowed) {
            ...
            p.sendMessage(color("  &#ff4444&l[DUEL] &fDuel sırasında sadece &e/ec &f, &e/pv 1 &f, &e/pv 2 &fkomutlarını kullanabilirsin!"));
        }
    }
