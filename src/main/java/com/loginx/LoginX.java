...
public class LoginX extends JavaPlugin implements Listener {
    ...
    /**
     * Günlük ödül anahtarı inşa eder — görünüşü "Ominous Bottle", mevcut kasa anahtarı sistemiyle
     * (keyTag) tam uyumludur. Sunucuda /kasaayarla event | vote | kit isimli kasalar kurulduğunda
     * bu anahtarlar o kasaları doğrudan açabilir.
     */
    private ItemStack buildDailyRewardKey(String kasaId, int amount) {
        ...
        ItemMeta meta = item.getItemMeta();
        meta.setDisplayName(color("&#ff9ecb&l⚷ &r&#ffffff&l" + kasaName + " Anahtarı"));
        List<String> lore = new ArrayList<>();
        lore.add(color("&#5a5a6a▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
        lore.add(color("&fSilvera &#ff9ecbGünlük Ödül &fSistemi &7ile kazanıldı."));
        lore.add(color(""));
        lore.add(color("&7Tür &8» &f" + kasaName));
        lore.add(color("&7Adet &8» &f" + amount));
        lore.add(color(""));
        lore.add(color("&#5fb87a▸ &7İlgili kasaya sağ tıklayarak kullan."));
        lore.add(color("&#5a5a6a▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
        meta.setLore(lore);
    ...
    private void openDailyRewardMenu(Player p) {
        ...
        String title = color("&#ffffff&lSilvera &#ff9ecb&l✦ Günlük Ödül ✦");
        ...
        infoMeta.setDisplayName(color("&#ff9ecb&l✦ Silvera Günlük Ödül ✦"));
        ...
        closeMeta.setDisplayName(color("&#ff4444&lKapat"));
    ...
    private ItemStack buildDailyRewardDayIcon(int day, int currentDay, boolean availableToday, long now, long nextClaim) {
        ...
        if (day < currentDay) {
            ...
            statusLine = color("&#5fb87a✔ Alındı");
        } else if (day == currentDay) {
            if (availableToday) {
                mat = isLastDay ? Material.ENDER_CHEST : Material.CHEST;
                statusLine = color("&#5fb87a✔ Ödül Alınabilir &8— &fTıkla ve Al!");
                clickable = true;
            } else {
                mat = Material.CLOCK;
                statusLine = color("&#e0a95f⏳ Bekleniyor &8(" + formatDuration(nextClaim - now) + "&8)");
            }
        } else {
            mat = Material.IRON_BARS;
            statusLine = color("&#ff6666🔒 Bu güne henüz ulaşmadın");
        }

        ItemStack item = new ItemStack(mat);
        ItemMeta meta = item.getItemMeta();
        String dayLabel = isLastDay ? ("Gün " + day + " &8— &#ff9ecb&lSon Gün") : ("Gün " + day);
        meta.setDisplayName(color("&#ffffff&l" + dayLabel));

        List<String> lore = new ArrayList<>();
        lore.add(color("&#5a5a6a▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
        lore.add(statusLine);
        lore.add(color(""));
        lore.add(color("&7İçerik &8»"));
        for (Map.Entry<String, Integer> ent : contents.entrySet()) {
            lore.add(color("&f  • " + ent.getValue() + "x &#ff9ecb" + dailyRewardKasaDisplayName(ent.getKey()) + " Anahtarı"));
        }
        lore.add(color(""));
        if (clickable) {
            lore.add(color("&#5fb87a▸ &fAlmak için tıkla!"));
        } else if (day == currentDay) {
            lore.add(color("&#e0a95f▸ &7Süre dolunca tekrar aç."));
        } else if (day < currentDay) {
            lore.add(color("&#5fb87a▸ &7Bu ödülü bu döngüde teslim aldın."));
        } else {
            lore.add(color("&#ff6666▸ &7Önceki günleri tamamlaman gerekiyor."));
        }
        lore.add(color("&#5a5a6a▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
        meta.setLore(lore);
    ...
    /** /kitkasası komutunu işler: tek hesap + tek IP başına tek seferlik, kalıcı olarak config'e kaydedilir. */
