...
public class LoginX extends JavaPlugin implements Listener {
    ...
    /**
     * Günlük ödül anahtarı inşa eder — görünüşü "Ominous Bottle", adı kesin olarak "Aminous Trial Key".
    ...
    private ItemStack buildDailyRewardKey(String kasaId, int amount) {
        ...
        ItemMeta meta = item.getItemMeta();
        meta.setDisplayName(buildRgbFlop("Aminous Trial Key", DAILY_REWARD_GRADIENT, false));
        List<String> lore = new ArrayList<>();
        ...
        lore.add(color(""));
        lore.add(color("&7Anahtar Türü &8» &fAminous Trial Key"));
    ...
    private ItemStack buildDailyRewardDayIcon(int day, int currentDay, boolean availableToday, long now, long nextClaim) {
        ...
        if (isLastDay) {
            ...
            lore.add(color("&#ff9ecbAminous Trial Key &7ödülleri hazırlandı."));
        }
        lore.add(color(""));
        lore.add(color("&7İçerik &8» &f" + totalKeys + " adet &#ff9ecbAminous Trial Key"));
        for (Map.Entry<String, Integer> ent : contents.entrySet()) {
            lore.add(color("&f  • " + ent.getValue() + "x &#ff9ecbAminous Trial Key &7(" + dailyRewardKasaDisplayName(ent.getKey()) + ")"));
        }
        lore.add(color(""));
        if (clickable) {
    ...
    private void handleKitKasasiCommand(Player p) {
        ...
        if (p.isOp()) {
            ...
            p.playSound(p.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 1f, 1.3f);
            p.sendTitle(color("&#ff9ecb&l✦ KIT KASASI ✦"), color("&fÖdülün envanterine eklendi!"), 5, 50, 10);
            p.sendMessage(color(""));
            p.sendMessage(color("  &#ff9ecb&l✦ Silvera &8» &fKit Kasası Anahtarı envanterine eklendi. &8(OP — sınırsız)"));
            p.sendMessage(color(""));
            return;
        }
        ...
        p.sendTitle(color("&#ff9ecb&l✦ KIT KASASI ✦"), color("&fÖdülün envanterine eklendi!"), 5, 50, 10);
        p.sendMessage(color(""));
        p.sendMessage(color("  &#ff9ecb&l✦ Silvera &8» &fKit Kasası Anahtarı envanterine eklendi."));
