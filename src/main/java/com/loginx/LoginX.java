...
public class LoginX extends JavaPlugin implements Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmdName.equals("kontal")) {
            ...
            // Oyuncuyu kont yerine işınla
            target.teleport(kontYeri);

            // Yetkiliyi de kendi yetkili yerine ışınla (ayarlıysa)
            Location adminLoc = kontYetkiliYeri.get(admin.getName());
            if (adminLoc != null) admin.teleport(adminLoc);
        ...
        // /kontyeryetkiliayarla (isim) — yetkili kendi ışınlanma yerini kaydeder
        if (cmdName.equals("kontyeryetkiliayarla")) {
            if (!(sender instanceof Player admin)) return true;
            if (!admin.hasPermission("loginx.admin")) { admin.sendMessage(color("&cYetkisiz.")); return true; }
            String hedef = args.length > 0 ? args[0] : admin.getName();
            kontYetkiliYeri.put(hedef, admin.getLocation().clone());
            getConfig().set("kontyetkili." + hedef, admin.getLocation());
            saveConfig();
            admin.sendMessage(color("&#00ff88&l[KONT] &f" + hedef + " &7için yetkili yeri ayarlandı."));
            return true;
        }

        // /kontyeryetkilisil (isim)
        if (cmdName.equals("kontyeryetkilisil")) {
            if (!(sender instanceof Player admin)) return true;
            if (!admin.hasPermission("loginx.admin")) { admin.sendMessage(color("&cYetkisiz.")); return true; }
            String hedef = args.length > 0 ? args[0] : admin.getName();
            ...
            saveConfig();
            admin.sendMessage(color("&#ff4444&l[KONT] &f" + hedef + " &7için yetkili yeri silindi."));
            return true;
        }

        // /izinver — sadece konsol veya OP — isim argümanı zorunlu
        if (cmdName.equals("izinver")) {
            if (sender instanceof Player p && !p.isOp()) { p.sendMessage(color("&cBu komut sadece konsol veya OP için.")); return true; }
            if (args.length < 1) { sender.sendMessage(color("&cKullanim: /izinver <oyuncu>")); return true; }
            Player target = Bukkit.getPlayer(args[0]);
            if (target == null) { sender.sendMessage(color("&cOyuncu bulunamadı: " + args[0])); return true; }
            izinliOyuncular.add(target.getUniqueId());
            getConfig().set("izinli." + target.getUniqueId().toString(), true); saveConfig();
            sender.sendMessage(color("&#00ff88&l[İZİN] &f" + target.getName() + " &7TNT/worldedit izni verildi."));
            target.sendMessage(color("&#00ff88&l[İZİN] &fSana TNT ve worldedit izni verildi!"));
            return true;
        }

        // /izinkaldır <oyuncu>
        if (cmdName.equals("izinkaldır") || cmdName.equals("izinkaldir")) {
            if (sender instanceof Player p && !p.isOp()) { p.sendMessage(color("&cBu komut sadece konsol veya OP için.")); return true; }
            if (args.length < 1) { sender.sendMessage(color("&cKullanim: /izinkaldır <oyuncu>")); return true; }
            Player target = Bukkit.getPlayer(args[0]);
            if (target != null) {
            ...
            sender.sendMessage(color("&#ff4444&l[İZİN] &f" + args[0] + " &7için izin kaldırıldı."));
            return true;
        }
    ...
    public void onPlayerDeath(PlayerDeathEvent e) {
        ...
        // ─── Totem patlaması — öldürenin ekranında sadece ses + minimal efekt ───
        if (killer != null) {
    ...
    private static final Object[][] ALL_EFFECTS = {
        ...
        {"elmas",     "Elmas",      "&#aaffee","Parlayan END_ROD dalgası",          Material.DIAMOND_SHARD},
        ...
        {"orak",      "Orak",       "&#445544","Köy + witch döngü efekti",          Material.SCUTE},
    ...
    // ─── Kont freeze — walkSpeed yerine event ile ───
    @EventHandler(priority = EventPriority.HIGHEST)
    public void onKontMoveFreeze(PlayerMoveEvent evM) {
        Player p = evM.getPlayer();
        if (!kontUntil.containsKey(p.getUniqueId())) return;
        Location from = evM.getFrom(); Location to = evM.getTo();
        if (to == null) return;
        if (from.getBlockX() != to.getBlockX() || from.getBlockY() != to.getBlockY() || from.getBlockZ() != to.getBlockZ()) {
            evM.setTo(from.clone().setDirection(to.getDirection()));
        }
    }
    ...
    public void onTNTPlace(org.bukkit.event.block.BlockPlaceEvent evB) {
        ...
        if (p.isOp() || p.hasPermission("loginx.admin")) return;
        if (izinliOyuncular.contains(p.getUniqueId())) return;
        Material mat = evB.getBlockPlaced().getType();
        if (mat == Material.TNT || mat == Material.MINECART || mat == Material.TNT_MINECART) {
            evB.setCancelled(true);
            p.sendMessage(color("&#ff4444&l[İZİN] &fBu sunucuda TNT/Minecart koyma izniniz yok!"));
        }
    }
    ...
    public void onTNTCommand(PlayerCommandPreprocessEvent evCmd2) {
        ...
        // WorldEdit ve TNT komutları engelle
        boolean blocked = cmd2.startsWith("//") || cmd2.startsWith("/fawe") || cmd2.startsWith("/we ")
            || cmd2.startsWith("/worldedit") || cmd2.startsWith("/tnt") || cmd2.startsWith("/cannon");
        if (blocked) {
