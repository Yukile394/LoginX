...
public class LoginX extends JavaPlugin implements Listener {
    ...
    // ─── NPC Sistemi ───
    // Tam player görünümlü NPC: gövde ArmorStand (ekipman ile), hologram yukarıda
    ...
    // NPC gövde entity'si: tam boyutlu ArmorStand (player ekipmanı takılı)
    private final HashMap<Integer, ArmorStand>          npcEntities     = new HashMap<>();
    // Ekstra görsel parçalar: bacaklar için küçük ArmorStand, kafa için skull
    ...
    // entity UUID → npc id (tıklama tespiti)
    private final HashMap<UUID, Integer>                entityUuidToNpc = new HashMap<>();
    ...
    // ─────────────────────────────────────────────
    //  NPC SİSTEMİ — YARDIMCI METODLAR
    // ─────────────────────────────────────────────

    /**
     * Tam player görünümlü NPC spawn eder.
     * Vücut: tam boyutlu ArmorStand + player ekipmanı (deri zırh renkli = görünmez)
     * Kafa: PLAYER_HEAD skull (skinOwner'ın skini)
     * Ölçek: scale < 1.0 → küçük stand, scale > 1.0 → birden fazla stand ile boyutlu görünüm
     * Hologram: baş üstünde tam konumda
     */
    private void spawnNPCEntity(SilveraNPC npc) {
        // Eski entity varsa temizle
        ArmorStand old = npcEntities.remove(npc.id);
        if (old != null) {
            entityUuidToNpc.remove(old.getUniqueId());
        ...
        // ── Ana gövde ArmorStand ──
        ArmorStand stand = (ArmorStand) loc.getWorld().spawnEntity(loc, EntityType.ARMOR_STAND);
        stand.setGravity(false);
        stand.setVisible(true);
        stand.setCustomNameVisible(false);
        // scale >= 1.0 → tam boyut, scale < 0.6 → küçük
        boolean useSmall = npc.scale < 0.6;
        stand.setSmall(useSmall);
        stand.setArms(true);
        ...
        // ── Skin: PLAYER_HEAD skull (skinOwner kişinin kafası) ──
        ItemStack skull = new ItemStack(Material.PLAYER_HEAD);
        SkullMeta sm = (SkullMeta) skull.getItemMeta();
        // OfflinePlayer üzerinden skin ata (Bukkit cache'den çeker)
        sm.setOwningPlayer(Bukkit.getOfflinePlayer(npc.skinOwner));
        // Texture henüz yüklenmediyse async fetch yap — yüklenince NPC yenilenir
        if (npc.skinTexture == null || npc.skinTexture.isEmpty()) {
            fetchSkinAsync(npc);
        }
        ...
        // ── Zırh: görünmez deri zırh (vücut doluluk hissi için) ──
        // Tamamen şeffaf deri zırh — sadece kafa görünür, vücut ArmorStand şeklinde
        // (İsteğe bağlı: elbiseleri de ekleyebiliriz)

        // ── NPC ID persistent data ──
        NamespacedKey npcKey = new NamespacedKey(this, "npc_id");
        stand.getPersistentDataContainer().set(npcKey, PersistentDataType.INTEGER, npc.id);

        entityUuidToNpc.put(stand.getUniqueId(), npc.id);
        npcEntities.put(npc.id, stand);

        // scale > 1.0 → ArmorStand'ı büyütmek için display entity ekle (1.19.4+)
        applyNPCScale(npc, stand);

        spawnNPCHologram(npc);
    }

    /** Scale uygula — 1.19.4+ Item Display entity ile gerçek boyut */
    @SuppressWarnings("deprecation")
    private void applyNPCScale(SilveraNPC npc, ArmorStand stand) {
        if (npc.scale == 1.0) return;
        try {
            // ItemDisplay veya BlockDisplay entity ile sarmalayarak scale ver
            // Bukkit 1.19.4+ destekler
            org.bukkit.entity.Display display = (org.bukkit.entity.Display)
                stand.getWorld().spawnEntity(stand.getLocation(), EntityType.ITEM_DISPLAY);
            display.setGravity(false);
            display.setInvulnerable(true);
            org.bukkit.util.Transformation t = display.getTransformation();
            float s = (float) npc.scale;
            ...
            // Display entity'yi ArmorStand'a passenger olarak ekle
            stand.addPassenger(display);
        } catch (Exception ignored) {
            // 1.19.4 altında bu API yok — sessizce geç
        }
    }

    /** Mojang API'dan skin texture/signature çek (async) */
    private void fetchSkinAsync(SilveraNPC npc) {
        new BukkitRunnable() {
            @Override public void run() {
                try {
                    URL url = new URL(
                    ...
                    con.setConnectTimeout(4000); con.setReadTimeout(4000);
                    if (con.getResponseCode() != 200) return;
                    // Basit JSON parse — bağımlılıksız
                    String body = new String(con.getInputStream().readAllBytes());
                    String uuid = extractJson(body, "id");
                    ...
                    con2.setConnectTimeout(4000); con2.setReadTimeout(4000);
                    if (con2.getResponseCode() != 200) return;
                    String body2 = new String(con2.getInputStream().readAllBytes());

                    // properties[0].value ve signature bul
                    String texture = extractJsonArray(body2, "value");
                    String signature = extractJsonArray(body2, "signature");
                    if (texture == null || texture.isEmpty()) return;

                    npc.skinTexture = texture;
                    npc.skinSignature = signature;

                    // Ana thread'de NPC'yi yenile
                    new BukkitRunnable() {
                        @Override public void run() {
    ...
    private String extractJsonArray(String json, String key) {
        // properties dizisindeki ilk eşleşmeyi bul
        String search = "\"" + key + "\":\"";
        int idx = json.indexOf(search);
        if (idx < 0) return null;
        int start = idx + search.length();
        // Base64 string içinde \\ olabilir
        StringBuilder sb = new StringBuilder();
    ...
    private void spawnNPCHologram(SilveraNPC npc) {
        ...
        // Hologram yüksekliği: tam player başının üstü
        // Normal boyut: standart ArmorStand kafası ~1.98 blok, hologram ~0.35 yukarı
        // scale ile orantılı
        double baseHeight = npc.scale < 0.6 ? 1.05 : 2.10;
        double hY = baseHeight * Math.max(0.3, npc.scale);
    ...
    private void removeNPC(int npcId) {
        npcMap.remove(npcId);
        ArmorStand as = npcEntities.remove(npcId);
        if (as != null) {
            entityUuidToNpc.remove(as.getUniqueId());
            // Passenger'ları da temizle (scale display entity)
            for (Entity passenger : as.getPassengers()) passenger.remove();
    ...
    public void onNPCInteract(org.bukkit.event.player.PlayerInteractEntityEvent ev) {
        ...
        // Hem ArmorStand'ı hem de olası display passenger'ı kontrol et
        Integer nid = entityUuidToNpc.get(clicked.getUniqueId());
        if (nid == null) {
            // Passenger olabilir — vehicle kontrol et
            if (clicked.getVehicle() != null) {
                nid = entityUuidToNpc.get(clicked.getVehicle().getUniqueId());
            }
        }
        if (nid == null) return;
        ev.setCancelled(true);
