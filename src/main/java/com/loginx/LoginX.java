...
public class LoginX extends JavaPlugin implements Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmdName.equals("1v1")) {
            ...
            // Hedef oyuncuya tıklanabilir davet mesajı — RGB flop kalın pembe-beyaz
            net.md_5.bungee.api.chat.TextComponent msg1 = new net.md_5.bungee.api.chat.TextComponent(
                color("  &#ff88cc&l⬛ &#ffaadd&l⬛ &#ffd4ee&l⬛ &#ffffff&l✔ KABUL ET &#ffd4ee&l⬛ &#ffaadd&l⬛ &#ff88cc&l⬛"));
            ...
            net.md_5.bungee.api.chat.TextComponent msg2 = new net.md_5.bungee.api.chat.TextComponent(
                color("  &#ff88cc&l⬛ &#ffaadd&l⬛ &#ffd4ee&l⬛ &#ffffff&l✘ REDDET  &#ffd4ee&l⬛ &#ffaadd&l⬛ &#ff88cc&l⬛"));
            ...
            target.sendMessage(color("  &#ff6600⚔ &r&#ffcc001v1 Daveti"));
            ...
            target.spigot().sendMessage(msg1, msg2);
            target.sendMessage(color(""));
        ...
        // /stashac — handled above as /stashaç alias

        // /stashkapa — stash'ı kapat
        if (cmdName.equals("stashkapa")) {
            if (!(sender instanceof Player p)) return true;
            if (!stashOpen.getOrDefault(p.getUniqueId(), false)) {
                p.sendMessage(color("&#00ccff&lStash &8» &cZaten kapalı!"));
                return true;
            }
            stashOpen.put(p.getUniqueId(), false);
            if (p.getOpenInventory().getTopInventory().getSize() == 27) {
            ...
            p.closeInventory();
            p.sendMessage(color("&#00ccff&lStash &8» &cStash kapatıldı. Ölürsen eşyaların düşmez."));
            return true;
        }
        ...
        // /stashaç — /stashac ile aynı (Türkçe alias)
        if (cmdName.equals("stashaç") || cmdName.equals("stashac")) {
            if (!(sender instanceof Player p)) return true;
            if (stashOpen.getOrDefault(p.getUniqueId(), false)) {
                p.sendMessage(color("&#00ccff&lStash &8» &fZaten açık!"));
                return true;
            }
            stashOpen.put(p.getUniqueId(), true);
            Inventory stashInv = Bukkit.createInventory(null, 27, color("&#00ccff&l✦ Stash ✦"));
            ...
            p.openInventory(stashInv);
            p.sendMessage(color("&#00ccff&lStash &8» &aStash açıldı. Ölürsen buradaki eşyalar düşer!"));
            return true;
        }
    ...
    public void onPlayerDeath(PlayerDeathEvent e) {
        ...
        if (hasItems) {
            // Öldüren oyuncunun kafası — RGB flop pembe-beyaz lore
            ItemStack skull = new ItemStack(Material.PLAYER_HEAD);
            org.bukkit.inventory.meta.SkullMeta sm = (org.bukkit.inventory.meta.SkullMeta) skull.getItemMeta();
            sm.setOwningPlayer(Bukkit.getOfflinePlayer(dead.getUniqueId()));
            // RGB flop isim
            String[] rgbCols = {"&#ff88cc","&#ffaadd","&#ffd4ee","&#ffffff","&#ffd4ee","&#ffaadd","&#ff88cc"};
            StringBuilder nameBuilder = new StringBuilder();
            String name = dead.getName();
            for (int i = 0; i < name.length(); i++) nameBuilder.append(rgbCols[i % rgbCols.length]).append(name.charAt(i));
            sm.setDisplayName(color(nameBuilder + " &r&#ffffff&l'nin Eşyaları"));
            List<String> skullLore = new ArrayList<>();
            skullLore.add(color("  &#ff88cc▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
            skullLore.add(color("  &#ffffff&oSağ tıkla → eşyaları al"));
            if (killer != null) skullLore.add(color("  &#ffaadd&oÖldüren: &#ffffff" + killer.getName()));
            skullLore.add(color("  &#ff88cc▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
            skullLore.add(color("&0DEATH_SKULL:" + dead.getUniqueId()));
            sm.setLore(skullLore);
            skull.setItemMeta(sm);

            // Eşyaları shulker'a koy
            ...
            for (ItemStack it : inv) {
                if (it != null && it.getType() != Material.AIR && slot < box.getInventory().getSize()) {
                    box.getInventory().setItem(slot++, it.clone());
                }
            }
            bsm.setBlockState(box);
            // Shulker'ı kafanın içine göm — aynı yere
            bsm.setDisplayName(color("&#00ccff" + dead.getName() + " &#ffffff- Eşyalar"));
            List<String> sl = new ArrayList<>();
            sl.add(color("&0DEATH_INV:" + dead.getUniqueId()));
            bsm.setLore(sl);
            shulker.setItemMeta(bsm);

            // Kafayı yere düşür (shulker içinde saklı)
            // Kafayı düşür — tıklayınca shulker içeriğini ver
            Location dropLoc = dead.getLocation().clone();
            org.bukkit.entity.Item droppedSkull = dead.getWorld().dropItem(dropLoc, skull);
            droppedSkull.setPickupDelay(40);
            // Shulker'ı gizli düşür (aynı yere)
            org.bukkit.entity.Item droppedShulker = dead.getWorld().dropItem(dropLoc, shulker);
            droppedShulker.setPickupDelay(Integer.MAX_VALUE); // Kimse toplayamaz — sadece kafa ile açılır
            droppedShulker.setInvulnerable(true);

            dead.getInventory().clear();
        }

        // ─── Stash varsa düşür ───
        if (stashOpen.getOrDefault(dead.getUniqueId(), false)) {
            // Stash AÇIKKEN → kafayı + shulker'ı kaldır, sadece oyuncunun kafası düşsün
            // Kafa zaten yukarıda düşürüldü (eşyalarla birlikte) — stash içeriğini de shulker'a ekle
            ItemStack[] stashContents = playerStash.get(dead.getUniqueId());
            if (stashContents != null) {
                // Stash içeriğini ayrı bir kafa+shulker çifti olarak düşür
                ItemStack stashSkull = new ItemStack(Material.PLAYER_HEAD);
                org.bukkit.inventory.meta.SkullMeta ssm = (org.bukkit.inventory.meta.SkullMeta) stashSkull.getItemMeta();
                ssm.setOwningPlayer(Bukkit.getOfflinePlayer(dead.getUniqueId()));
                String[] rgbColsS = {"&#ff88cc","&#ffaadd","&#ffd4ee","&#ffffff","&#ffd4ee","&#ffaadd","&#ff88cc"};
                StringBuilder snb = new StringBuilder();
                String sname = dead.getName();
                for (int i = 0; i < sname.length(); i++) snb.append(rgbColsS[i % rgbColsS.length]).append(sname.charAt(i));
                ssm.setDisplayName(color(snb + " &r&#00ccff&l'nin Stash'ı"));
                List<String> sSkullLore = new ArrayList<>();
                sSkullLore.add(color("  &#00ccff▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
                sSkullLore.add(color("  &#ffffff&oSağ tıkla → stash eşyalarını al"));
                sSkullLore.add(color("  &#00ccff▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
                sSkullLore.add(color("&0DEATH_SKULL:" + dead.getUniqueId() + ":stash"));
                ssm.setLore(sSkullLore);
                stashSkull.setItemMeta(ssm);

                ItemStack stashShulker = new ItemStack(Material.LIGHT_BLUE_SHULKER_BOX);
                BlockStateMeta sbsm = (BlockStateMeta) stashShulker.getItemMeta();
                ShulkerBox sbox = (ShulkerBox) sbsm.getBlockState();
                for (int i = 0; i < Math.min(stashContents.length, sbox.getInventory().getSize()); i++) {
                    if (stashContents[i] != null) sbox.getInventory().setItem(i, stashContents[i].clone());
                }
                sbsm.setBlockState(sbox);
                sbsm.setDisplayName(color("&#00ccff" + dead.getName() + " &#ffffff- Stash"));
                List<String> stashL = new ArrayList<>();
                stashL.add(color("&0DEATH_STASH:" + dead.getUniqueId()));
                sbsm.setLore(stashL);
                stashShulker.setItemMeta(sbsm);

                Location dropLoc2 = dead.getLocation().clone();
                org.bukkit.entity.Item droppedStashSkull = dead.getWorld().dropItem(dropLoc2, stashSkull);
                droppedStashSkull.setPickupDelay(40);
                org.bukkit.entity.Item droppedStashShulker = dead.getWorld().dropItem(dropLoc2, stashShulker);
                droppedStashShulker.setPickupDelay(Integer.MAX_VALUE);
                droppedStashShulker.setInvulnerable(true);
            }
            playerStash.remove(dead.getUniqueId());
            stashOpen.put(dead.getUniqueId(), false);
        } else {
            // Stash KAPALI → stash içeriğini normal item olarak tek noktaya dök (dağılmadan)
            ItemStack[] stashContents = playerStash.get(dead.getUniqueId());
            if (stashContents != null) {
                Location dropLoc3 = dead.getLocation().clone();
                for (ItemStack it : stashContents) {
                    if (it != null && it.getType() != Material.AIR) {
                        org.bukkit.entity.Item dropped = dead.getWorld().dropItem(dropLoc3, it.clone());
                        dropped.setVelocity(new org.bukkit.util.Vector(0, 0, 0)); // dağılmasın
                    ...
                    if (dr != null) {
                        if (e.isLeftClick()) {
                            ...
                            if (t != null && t.isOnline()) {
                                t.getInventory().setContents(dr.items);
                                p.sendMessage(color("&aIade edildi."));
    ...
    public void onChat(org.bukkit.event.player.AsyncPlayerChatEvent ev) {
        ...
        if (spamMuteUntil > now || kufurMuteUntil > now) {
            ...
            p.sendMessage(buildFlopMsg("&lMUTE") + color(" &7— " + min + " dk " + sec + " sn kaldı."));
            return;
        }
        ...
        if (hasKufur) {
            ...
            if (warnK >= 5) {
                ...
                p.sendMessage(buildFlopMsg("&lKÜFÜR") + color(" &710 dk mute aldın!"));
                for (Player on : Bukkit.getOnlinePlayers())
                    if (on.hasPermission("loginx.admin"))
                        on.sendMessage(color("&#ff4444&l[KÜFÜR] &f" + p.getName() + " &710 dk mute."));
            } else {
                p.sendMessage(buildFlopMsg("&lKÜFÜR") + color(" &7uyarı &e" + warnK + "/5"));
            }
            return;
        }
        ...
        if (times.size() >= SPAM_THRESHOLD) {
            ...
            if (warnS >= 5) {
                ...
                times.clear();
                p.sendMessage(buildFlopMsg("&lSPAM") + color(" &7yaptın — 10 dk mute aldın!"));
                for (Player on : Bukkit.getOnlinePlayers())
                    if (on.hasPermission("loginx.admin"))
                        on.sendMessage(color("&#ff4444&l[SPAM] &f" + p.getName() + " &710 dk mute."));
            } else {
                p.sendMessage(buildFlopMsg("&lSPAM") + color(" &7yapma uyarı &e" + warnS + "/5"));
            }
        }
    }

    /** RGB flop beyaz-pembe, sadece etiket kalın */
    private String buildFlopMsg(String tag) {
        String[] cols = {"&#ff88cc","&#ffaadd","&#ffd4ee","&#ffffff","&#ffd4ee","&#ffaadd","&#ff88cc"};
        StringBuilder sb = new StringBuilder();
        String stripped = org.bukkit.ChatColor.stripColor(color(tag));
        for (int i = 0; i < stripped.length(); i++) sb.append(cols[i % cols.length]).append(stripped.charAt(i));
        return color(sb.toString()) + color(" &r&#ffbbdd" + tag);
    }
