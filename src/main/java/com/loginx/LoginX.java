...
public class LoginX extends JavaPlugin implements Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmdName.equals("npskin")) {
            ...
            // ArmorStand kafasına player kafası olarak güncelle
            ArmorStand as = npcEntities.get(nid);
            if (as != null) {
                ItemStack skull = new ItemStack(Material.PLAYER_HEAD);
                SkullMeta sm = (SkullMeta) skull.getItemMeta();
                // SkinsRestorer'ın değiştirdiği profil yoksa Bukkit offline player ile dene
                org.bukkit.OfflinePlayer op = Bukkit.getOfflinePlayer(args[1]);
                sm.setOwningPlayer(op);
                skull.setItemMeta(sm);
                as.getEquipment().setHelmet(skull);
            }
            p.sendMessage(buildSilveraTag() + color(" &fNPC &e#" + nid + " &7skini: &a" + args[1]));
            return true;
        }
        ...
        if (cmdName.equals("npdondur")) {
            ...
            ArmorStand as = npcEntities.get(nid);
            if (as != null) {
                Location tpLoc = npc.loc.clone();
                tpLoc.setYaw(npc.yaw);
                as.teleport(tpLoc);
            }
            updateNPCHologram(npc);
        ...
        // /npboyut <id> <boyut> — NPC büyüklüğü
        if (cmdName.equals("npboyut")) {
            ...
            npc.scale = size;
            ArmorStand as = npcEntities.get(nid);
            if (as != null) as.setSmall(size < 0.7);
    ...
    private void spawnNPCEntity(SilveraNPC npc) {
        ...
        // Tam boyutlu ArmorStand — player gibi görünmesi için ekipman takılır
        ArmorStand stand = (ArmorStand) loc.getWorld().spawnEntity(loc, EntityType.ARMOR_STAND);
        stand.setGravity(false);
        stand.setVisible(true);          // vücut görünür
        stand.setCustomNameVisible(false);
        stand.setSmall(npc.scale < 0.7);
        stand.setArms(true);             // kollar açık
        stand.setBasePlate(false);       // altında plaka yok
        stand.setMarker(false);          // tıklanabilir
        stand.setInvulnerable(true);
        stand.setCanPickupItems(false);

        // Kafaya oluşturan kişinin player kafasını tak (skin için)
        ItemStack skull = new ItemStack(Material.PLAYER_HEAD);
        SkullMeta sm = (SkullMeta) skull.getItemMeta();
        try {
            org.bukkit.OfflinePlayer op = Bukkit.getOfflinePlayer(npc.skinOwner);
            sm.setOwningPlayer(op);
        } catch (Exception ignored) {}
        skull.setItemMeta(sm);
        ...
        // NPC ID'yi persistent data olarak sakla (tıklama tespiti için)
    ...
    private void spawnNPCHologram(SilveraNPC npc) {
        ...
        // Hologram NPC'nin 1.2 blok yukarısında — shift+ortası seviyesinde
        double hY = 1.55;
    ...
    // NPC'ye sağ tıklama — komut çalıştır (1.9 sn cooldown, dupe önlemi)
    ...
    public void onNPCInteract(org.bukkit.event.player.PlayerInteractEntityEvent ev) {
        ...
        UUID uid = p.getUniqueId();
        long now = System.currentTimeMillis();
        // 1.9 saniye cooldown — dupe önlemi
        ...
        // Komutları oyuncu adına çalıştır
