...
public class LoginX extends JavaPlugin implements Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmdName.equals("bakima")) {
            if (!(sender instanceof Player p)) return true;
            if (!p.hasPermission("loginx.admin")) { p.sendMessage(color("&cYetkisiz.")); return true; }
            bakimModu = true;
            for (Player on : new ArrayList<>(Bukkit.getOnlinePlayers())) {
                if (!on.hasPermission("loginx.admin")) {
                    on.kickPlayer(color("&#ff4444&lSilvera\n&#ffffff&lSunucu Kısa Süreliğine Bakımdadır.\n&#ffcc00&lYakında Geri Döneceğiz!"));
                }
            }
            for (Player on : Bukkit.getOnlinePlayers())
                on.sendMessage(color("&#ff4444&l[Silvera] &fSunucu bakım moduna alındı. Opsuz oyuncular atıldı."));
            return true;
        }
        ...
        if (cmdName.equals("bakimdancikar")) {
            ...
            bakimModu = false;
            for (Player on : Bukkit.getOnlinePlayers())
                on.sendMessage(color("&#00ff88&l[Silvera] &fSunucu bakım modundan çıkarıldı. Oyuncular girebilir."));
            return true;
        }
        ...
        if (cmdName.equals("bakimaoyuncu")) {
            ...
            target.kickPlayer(color("&#ff4444&lSilvera\n&#ffffff&lSunucu Kısa Süreliğine Bakımdadır.\n&#ffcc00&lYakında Geri Döneceğiz!"));
            p.sendMessage(color("&#ffcc00&l[Silvera] &f" + args[0] + " &7bakım nedeniyle atıldı."));
            return true;
        }
    ...
    public void onJoin(PlayerJoinEvent e) {
        ...
        if (bakimModu && !p.hasPermission("loginx.admin")) {
            new BukkitRunnable() {
                @Override public void run() {
                    p.kickPlayer(color("&#ff4444&lSilvera\n&#ffffff&lSunucu Kısa Süreliğine Bakımdadır.\n&#ffcc00&lYakında Geri Döneceğiz!"));
                }
            }.runTaskLater(this, 2L);
            return;
        }
        ...
        if (bakimModu && p.hasPermission("loginx.admin")) {
            new BukkitRunnable() {
                @Override public void run() {
                    ...
                    p.sendTitle(
                        color("&#ff4444&l⚠ BAKIM MODU AKTİF ⚠"),
                        color("&#ffcc00Sunucu bakım modunda. Oyuncular giremez."),
                        5, 80, 20);
                }
            }.runTaskLater(this, 20L);
        } else if (!bakimModu) {
    ...
    public void onChat(org.bukkit.event.player.AsyncPlayerChatEvent ev) {
        ...
        if (spamMuteUntil > now || kufurMuteUntil > now) {
            ...
            p.sendMessage(color("&#ff4444&l[Silvera] &cMute &7— " + min + " dk " + sec + " sn kaldı."));
            return;
        }
        ...
        if (hasKufur) {
            ...
            if (warnK >= 5) {
                ...
                p.sendMessage(color("&#ff4444&l[Silvera] &cKüfür Uyarısı &75/5 &8— &c10 dakika mute aldın!"));
                for (Player on : Bukkit.getOnlinePlayers())
                    if (on.hasPermission("loginx.admin"))
                        on.sendMessage(color("&#ff4444&l[Silvera/KÜFÜR] &f" + p.getName() + " &710 dk mute aldı."));
            } else {
                p.sendMessage(color("&#ff4444&l[Silvera] &cKüfür Uyarısı &7" + warnK + "/5 &8— &fKüfür etme!"));
            }
            return;
        }
        ...
        if (times.size() >= SPAM_THRESHOLD) {
            ...
            if (warnS >= 5) {
                ...
                times.clear();
                p.sendMessage(color("&#ff4444&l[Silvera] &cSpam Uyarısı &75/5 &8— &c10 dakika mute aldın!"));
                for (Player on : Bukkit.getOnlinePlayers())
                    if (on.hasPermission("loginx.admin"))
                        on.sendMessage(color("&#ff4444&l[Silvera/SPAM] &f" + p.getName() + " &710 dk mute aldı."));
            } else {
                p.sendMessage(color("&#ff4444&l[Silvera] &cSpam Uyarısı &7" + warnS + "/5 &8— &fYavaş mesaj at!"));
            }
        }
    }

    /** RGB flop kırmızı-beyaz Silvera etiketi */
    private String buildFlopMsg(String tag) {
        return color("&#ff4444&l[Silvera] " + tag);
    }
    ...
    private void giveIadeShulker(Player target, ItemStack[] items, String adminName, String playerName, String recordId) {
        ...
        // Mavi RGB flop animasyonlu "Silvera İade" ismi
        String[] blueCols = {"&#0055ff","&#0077ff","&#0099ff","&#00bbff","&#00ddff","&#00ffff","&#00ddff","&#00bbff"};
        ...
        sl.add(color("  &#0099ff▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
        sl.add(color("  &7İade Eden:  &e" + adminName));
        sl.add(color("  &7İade Alan:  &f" + playerName));
        sl.add(color("  &7Kayıt ID:   &8#" + recordId));
        sl.add(color("  &#0099ff▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
        sl.add(color("  &#00ccff» &fBasınca içindeki eşyaları al"));
        sl.add(color("  &#0099ff▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"));
        bsm.setLore(sl);
