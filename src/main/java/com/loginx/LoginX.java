...
public class LoginX extends JavaPlugin implements Listener {
    ...
    private static class SilveraNPC {
        ...
        double scale; String skinName; List<String> commands;
        SilveraNPC(int id, Location loc) {
            this.id = id; this.loc = loc.clone(); this.hologramName = "NPC #" + id;
            this.yaw = loc.getYaw(); this.pitch = loc.getPitch();
            this.scale = 1.0; this.skinName = null; this.commands = new ArrayList<>();
        }
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmdName.equals("npolustur")) {
            if (!(sender instanceof Player p)) return true;
            if (!p.hasPermission("loginx.admin")) { p.sendMessage(color("&cYetkisiz.")); return true; }
            Block target2 = p.getTargetBlockExact(5);
            Location npcLoc = target2 != null ? target2.getLocation().add(0.5, 1, 0.5) : p.getLocation().clone();
            npcLoc.setYaw(p.getLocation().getYaw());
            npcLoc.setPitch(p.getLocation().getPitch());
            int nid = npcIdCounter++;
            SilveraNPC npc = new SilveraNPC(nid, npcLoc);
            npcMap.put(nid, npc);
            spawnNPCEntity(npc);
            p.sendMessage(buildSilveraTag() + color(" &fNPC &e#" + nid + " &aoluşturuldu."));
            return true;
        }
        ...
        if (cmdName.equals("nprename")) {
            if (!(sender instanceof Player p)) return true;
            if (!p.hasPermission("loginx.admin")) { p.sendMessage(color("&cYetkisiz.")); return true; }
            if (args.length < 2) { p.sendMessage(color("&cKullanim: /nprename <id> <isim>")); return true; }
            int nid; try { nid = Integer.parseInt(args[0]); } catch (Exception ex) { p.sendMessage(color("&cGeçersiz ID.")); return true; }
            SilveraNPC npc = npcMap.get(nid);
            if (npc == null) { p.sendMessage(color("&cNPC bulunamadı: #" + nid)); return true; }
            String newName = String.join(" ", Arrays.copyOfRange(args, 1, args.length));
            npc.hologramName = color(newName);
            updateNPCHologram(npc);
            p.sendMessage(buildSilveraTag() + color(" &fNPC &e#" + nid + " &7ismi güncellendi: " + color(newName)));
            return true;
        }
        ...
        if (cmdName.equals("npskin")) {
            ...
            if (npc == null) { p.sendMessage(color("&cNPC bulunamadı: #" + nid)); return true; }
            npc.skinName = args[1];
            // SkinsRestorer ile değiştir
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "skin set " + args[1] + " " + args[1]);
            p.sendMessage(buildSilveraTag() + color(" &fNPC &e#" + nid + " &7skin: &a" + args[1] + " &7(SkinsRestorer ile)"));
            return true;
        }
        ...
        if (cmdName.equals("npdondur")) {
            ...
            SilveraNPC npc = npcMap.get(nid);
            if (npc == null) { p.sendMessage(color("&cNPC bulunamadı: #" + nid)); return true; }
            // Oyuncunun durduğu yere ve baktığı yöne göre kilitle
            npc.loc = p.getLocation().clone();
            npc.yaw = p.getLocation().getYaw();
            npc.pitch = p.getLocation().getPitch();
            ArmorStand as = npcEntities.get(nid);
            if (as != null) {
                as.teleport(npc.loc);
                as.setHeadPose(new org.bukkit.util.EulerAngle(Math.toRadians(npc.pitch), 0, 0));
            }
            updateNPCHologram(npc);
            p.sendMessage(buildSilveraTag() + color(" &fNPC &e#" + nid + " &7durduruldu ve konuma kilitlendi."));
            return true;
        }
        ...
        if (cmdName.equals("npboyut")) {
            if (!(sender instanceof Player p)) return true;
            if (!p.hasPermission("loginx.admin")) { p.sendMessage(color("&cYetkisiz.")); return true; }
            if (args.length < 2) { p.sendMessage(color("&cKullanim: /npboyut <id> <boyut>")); return true; }
            int nid; try { nid = Integer.parseInt(args[0]); } catch (Exception ex) { p.sendMessage(color("&cGeçersiz ID.")); return true; }
            SilveraNPC npc = npcMap.get(nid);
            if (npc == null) { p.sendMessage(color("&cNPC bulunamadı: #" + nid)); return true; }
            double size; try { size = Double.parseDouble(args[1]); } catch (Exception ex) { p.sendMessage(color("&cGeçersiz boyut.")); return true; }
            npc.scale = size;
            ArmorStand as = npcEntities.get(nid);
            if (as != null) as.setSmall(size < 0.6);
            p.sendMessage(buildSilveraTag() + color(" &fNPC &e#" + nid + " &7boyutu: &a" + size));
            return true;
        }
        ...
        if (cmdName.equals("npsil")) {
            ...
            int nid; try { nid = Integer.parseInt(args[0]); } catch (Exception ex) { p.sendMessage(color("&cGeçersiz ID.")); return true; }
            removeNPC(nid);
            p.sendMessage(buildSilveraTag() + color(" &fNPC &e#" + nid + " &c silindi (restart'ta geri gelmez)."));
            return true;
        }
        ...
        if (cmdName.equals("npkomutver")) {
            if (!(sender instanceof Player p)) return true;
            if (!p.hasPermission("loginx.admin")) { p.sendMessage(color("&cYetkisiz.")); return true; }
            if (args.length < 2) { p.sendMessage(color("&cKullanim: /npkomutver <id> <komut örn: /kit>")); return true; }
            int nid; try { nid = Integer.parseInt(args[0]); } catch (Exception ex) { p.sendMessage(color("&cGeçersiz ID.")); return true; }
            SilveraNPC npc = npcMap.get(nid);
            if (npc == null) { p.sendMessage(color("&cNPC bulunamadı: #" + nid)); return true; }
            String cmd2 = String.join(" ", Arrays.copyOfRange(args, 1, args.length));
            npc.commands.add(cmd2);
            p.sendMessage(buildSilveraTag() + color(" &fNPC &e#" + nid + " &7komut eklendi: &a" + cmd2));
            return true;
        }
    ...
    private void spawnNPCEntity(SilveraNPC npc) {
        ...
        stand.setSmall(npc.scale < 0.6);
        stand.setArms(true);
        stand.setBasePlate(false);
        stand.setMarker(false);
        // Yaw'ı uygula
        org.bukkit.util.Vector dir = new org.bukkit.util.Vector(
            -Math.sin(Math.toRadians(npc.yaw)), 0, Math.cos(Math.toRadians(npc.yaw)));
        stand.setVelocity(new org.bukkit.util.Vector(0, 0, 0));
        // Persistent data ile id sakla
        stand.getPersistentDataContainer().set(
            new NamespacedKey(this, "npc_id"), PersistentDataType.INTEGER, npc.id);
    ...
    private void spawnNPCHologram(SilveraNPC npc) {
        ...
        // Hologram 1 blok yukarıda, basamak ortası
        double hY = 2.3;
        ArmorStand holo = (ArmorStand) loc.getWorld().spawnEntity(loc.clone().add(0, hY, 0), EntityType.ARMOR_STAND);
        ...
        holo.getPersistentDataContainer().set(
            new NamespacedKey(this, "npc_holo"), PersistentDataType.INTEGER, npc.id);
        holograms.add(holo);
    ...
    private void updateNPCHologram(SilveraNPC npc) {
        ...
        if (holos != null && !holos.isEmpty()) {
            holos.get(0).setCustomName(color(npc.hologramName));
        } else {
            spawnNPCHologram(npc);
        }
    }
    ...
    // NPC'ye sağ tıklama — komut çalıştır (1.9 sn cooldown)
    ...
    public void onNPCInteract(org.bukkit.event.player.PlayerInteractEntityEvent ev) {
        ...
        if (!as.getPersistentDataContainer().has(new NamespacedKey(this, "npc_id"), PersistentDataType.INTEGER)) return;
        int nid = as.getPersistentDataContainer().get(new NamespacedKey(this, "npc_id"), PersistentDataType.INTEGER);
        SilveraNPC npc = npcMap.get(nid);
        if (npc == null) return;
        ev.setCancelled(true);
        Player p = ev.getPlayer();
        ...
        // Komutları çalıştır
        for (String cmd3 : npc.commands) {
            String finalCmd = cmd3.startsWith("/") ? cmd3.substring(1) : cmd3;
            Bukkit.dispatchCommand(p, finalCmd.replace("{player}", p.getName()));
        }
    }
