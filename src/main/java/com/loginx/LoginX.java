...
public class LoginX extends JavaPlugin implements Listener {
    ...
    public void onChat(org.bukkit.event.player.AsyncPlayerChatEvent ev) {
        ...
        if (spamMuteUntil > now || kufurMuteUntil > now) {
            ...
            p.sendMessage(color("&#ff4444 ✖ &r&fSusturuldun &8— &e" + min + " dk " + sec + " sn &fkaldı."));
            return;
        }
        ...
        if (hasKufur) {
            ...
            if (warnK >= 5) {
                ...
                // Uyarı mesajı — "KÜFÜR" kalın, rest ince
                p.sendMessage(color("&#ff4444&l⚠ KÜFÜR &r&#ffaaaa10 dakika susturuldun!"));
                for (Player on : Bukkit.getOnlinePlayers())
                    if (on.hasPermission("loginx.admin"))
                        on.sendMessage(color("&#ff4444&l[KÜFÜR] &f" + p.getName() + " &7→ 10 dk mute."));
            } else {
                p.sendMessage(color("&#ff4444&l⚠ KÜFÜR &r&#ffaaaa yapma uyarı &e&l" + warnK + "/5"));
            }
            return;
        }
        ...
        if (times.size() >= SPAM_THRESHOLD) {
            ...
            if (warnS >= 5) {
                ...
                times.clear();
                p.sendMessage(color("&#ff4444&l⚠ SPAM &r&#ffaaaa yaptın, 10 dakika susturuldun!"));
                for (Player on : Bukkit.getOnlinePlayers())
                    if (on.hasPermission("loginx.admin"))
                        on.sendMessage(color("&#ff4444&l[SPAM] &f" + p.getName() + " &7→ 10 dk mute."));
            } else {
                p.sendMessage(color("&#ff4444&l⚠ SPAM &r&#ffaaaa yapma uyarı &e&l" + warnS + "/5"));
            }
        }
    }

    /** Artık kullanılmıyor — eski flop renk sistemi */
    private String buildFlopMsg(String tag) {
        return color("&#ff4444&l⚠ " + tag.replace("&l","") + " &r&#ffaaaa");
    }
