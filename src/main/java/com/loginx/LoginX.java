...
public class LoginX extends JavaPlugin implements Listener {
    ...
    private void dropDuelShulker(Player p, Location loc) {
        ...
        for (ItemStack it : p.getInventory().getContents()) {
            if (it != null && it.getType() != Material.AIR) all.add(it.clone());
        }
        ...
        while (idx < all.size()) {
            ...
            int filled = 0;
            while (idx < all.size() && filled < boxInv.getSize()) {
                boxInv.setItem(filled, all.get(idx));
                idx++; filled++;
            }
            bsm.setBlockState(box);
            shulkerItem.setItemMeta(bsm);

            org.bukkit.entity.Item dropped = loc.getWorld().dropItem(loc, shulkerItem);
