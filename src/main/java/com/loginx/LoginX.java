...
import net.citizensnpcs.api.trait.trait.MobSize;
...
public class LoginX extends JavaPlugin implements Listener {
    ...
    private static class SilveraNPC {
        ...
        // Citizens NPC ID (plugin sistemindeki ID'den farklı)
        int citizensId = -1;
    ...
    private void spawnNPCEntity(SilveraNPC npc) {
        ...
        // ── Skin ──
        try {
            net.citizensnpcs.api.trait.trait.Owner ownerTrait =
                citizensNPC.getOrAddTrait(net.citizensnpcs.api.trait.trait.Owner.class);
            // Skin için player adını kullan (Citizens kendi skin fetch'ini yapar)
        } catch (Exception ignored) {}
        ...
        // ── Entity'nin yüzünü sabitle (bakmayı devre dışı bırak) ──
        try {
            net.citizensnpcs.api.trait.LookClose lookClose =
                citizensNPC.getOrAddTrait(net.citizensnpcs.api.trait.LookClose.class);
            lookClose.setLookClose(false);
        } catch (Exception ignored) {}
    ...
    /** Citizens NPC'ye scale uygula */
    private void applyNPCScale(NPC citizensNPC, double scale) {
        if (scale == 1.0) return;
        try {
            // Citizens 2.0.30+ MobSize trait ile scale
            MobSize sizeTrait = citizensNPC.getOrAddTrait(MobSize.class);
            // MobSize trait scale desteği Citizens versiyonuna göre değişir
            // Alternatif: entity attribute ile
        } catch (Exception ignored) {}
        // Entity üzerinden attribute ile dene (Paper/Spigot 1.20.5+)
        try {
            Entity ent = citizensNPC.getEntity();
            if (ent instanceof LivingEntity le) {
                var attr = le.getAttribute(org.bukkit.attribute.Attribute.GENERIC_SCALE);
